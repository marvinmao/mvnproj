...
public class SpringQueryTest {
	...
	public void testSLSOper() {
		Query query = query(where(null)).with(new Sort(Sort.Direction.ASC, "username")).limit(1).skip(2);
		
//		Pageable pageableRequest = new PageRequest(0, 2);
//		Query query = query(where(null)).with(new Sort(Sort.Direction.ASC, "username")).with(pageableRequest);
		List<User> find = tempelate.find(query, User.class);
		printUsers(find);
...
//备注：数组中对象数据要符合查询对象里面所有的字段，$全元素匹配，和顺序无关；
	...
	public void testObjArray3() throws ParseException {
		Criteria is = where("author").is("lison5");
		Criteria regex = where("content").regex(".*苍老师.*");
		Criteria andOperator = new Criteria().andOperator(is,regex);
